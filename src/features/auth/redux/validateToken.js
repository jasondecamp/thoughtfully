...
export function validateToken(args = {}) {
  return (dispatch) => { // optionally you can have getState as the second argument
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      const doRequest = axios.get('/password_reset',{params:args});
      doRequest.then(
        (res) => {
          dispatch({
        ...
        (err) => {
          dispatch({
            type: AUTH_VALIDATE_TOKEN_FAILURE,
            data: { error: err },
          });
          reject(err);
        },
...
export function dismissValidateTokenError() {
  return {
...
export function reducer(state, action) {
  switch (action.type) {
    case AUTH_VALIDATE_TOKEN_BEGIN:
    ...
    case AUTH_VALIDATE_TOKEN_SUCCESS:
    ...
    case AUTH_VALIDATE_TOKEN_FAILURE:
      // The request is failed
      return {
        ...state,
        validateTokenPending: false,
        validateTokenError: action.data.error,
      };

    case AUTH_VALIDATE_TOKEN_DISMISS_ERROR:
